...
function parseEsmtpParams(tail) {
  ...
  for (let i = 0; i < parts.length; i++) {
    ...
    if (eq === -1) {
      ...
      if (tk === 'REQUIRETLS') { params.requiretls = true; continue; }
      params[tk] = true;
      continue;
    }
    ...
    if (k === 'ENVID') { params.envid = decodeXtext(v); continue; }

    // RFC 3461 §4.1 — NOTIFY is a comma-separated list of event classes
    // controlling when DSNs should be generated. NEVER is exclusive —
    // if present, no other class may accompany it. We normalize to an
    // object for the 'rcpt' event so developer code doesn't have to parse.
    //   params.notify = { never, success, failure, delay }   (bool flags)
    if (k === 'NOTIFY') {
      let flags = { never: false, success: false, failure: false, delay: false };
      let tokens = v.split(',');
      for (let j = 0; j < tokens.length; j++) {
        let tk = tokens[j].trim().toUpperCase();
        if      (tk === 'NEVER')   flags.never   = true;
        else if (tk === 'SUCCESS') flags.success = true;
        else if (tk === 'FAILURE') flags.failure = true;
        else if (tk === 'DELAY')   flags.delay   = true;
      }
      params.notify = flags;
      continue;
    }

    // RFC 3461 §4.2 — ORCPT is the "original recipient" address, preserved
    // across forwarding so the sender learns which rcpt failed. Format:
    //   ORCPT=<addr-type>;<addr>
    // where addr-type is typically 'rfc822' (with xtext encoding for non-ASCII).
    if (k === 'ORCPT') {
      let semi = v.indexOf(';');
      if (semi > 0) {
        params.orcpt = { addrType: v.substring(0, semi).toLowerCase(), addr: decodeXtext(v.substring(semi + 1)) };
      } else {
...
// RFC 3461 §4 xtext decoder — "+XX" hex-escape for non-printable bytes in
// ENVID and ORCPT values. Leaves the rest alone.
function decodeXtext(s) {
